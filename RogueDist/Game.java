...
/*************************************************************************
 * Legacy comments from first authors Compilation: javac Game.java Execution: java Game < input.txt Dependencies:
 * Dungeon.java Site.java In.java Monster.java Rogue.java
 * 
 * April 2011 J. Smith -- fixed variable names, dependencies (removed N, this is stored in dungeon)
 * 
 * May 2012 by J. Smith Fudge for this year: Added methods to add a Monster and Rogue to the game Note: I really want to
 * undo the relationship between Creatures and Game where Game has an instance of Creature and vice versa. AFTER class
 * presentations and BEFORE distributing next year MAKE THIS CHANGE.
 * 
 * 
 * May 2013 by J. Smith - switched to 2013 GraphInterface class from SmallWorld problem - use a MoveFinder instead of
 * Creature - change moveMonster and moveRogue to moveCreature(Creature x)
 * 
	...
	public String toString() {
		...
		for (int i = 0; i < dungeon.size(); i++) {
			for (int j = 0; j < dungeon.size(); j++) {
				...
				if (rogueSite.equals(monsterSite) && rogueSite.equals(site))
					s += "* ";
